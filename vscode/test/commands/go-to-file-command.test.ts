...
import { RepositoryInfo, SelectedRange, UrlInfo } from '../../src/types';
import { toSelection } from '../../src/utilities';
import { Directory, getGitService, matchUri, remote, repository } from '../helpers';
...
describe('GoToFileLinkCommand', () => {
    ...
    async function setupFolder(folder: Folder, path: string): Promise<void> {
        if (folder.repository) {
            repositories.push({
                repository: repository({ rootUri: Uri.file(path) }),
                remote: remote(folder.repository, 'origin')
            });
        }
